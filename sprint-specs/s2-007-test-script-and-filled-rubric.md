# CDW Sprint S2_007 — Test Script + Filled Rubric
...
Produce the two study artifacts that S2_008 consumes: the filled rubric with specific planted facts and expected answers, and the 120-turn scripted user-side conversation. Per pre-registration discipline, the rubric specifics are locked before the script is written. The script is written against the rubric — not the other way around.
...
1. `experiments/study_002/rubric_filled.md` — rubric with specific planted facts and expected answers
2. `experiments/study_002/script.json` — 120-turn scripted user message sequence

Both files committed before S2_008 begins.

---

## Script Design

### Topics and Planted Facts

**Topic A — Structural Civil Engineering (turns 1–30)**
Planted in turns 1–5 (early position, token ~0–8,500).

| Fact | Value |
|------|-------|
| Bridge project name | Halcyon Crossing |
...
- Rule 1: Always format lists of technical specifications as a numbered list
- Rule 2: Always conclude structural recommendations with a risk classification: (Risk: Low), (Risk: Medium), or (Risk: High)

**Topic B — Renaissance Art History (turns 31–60)**
Planted in turns 55–65 (middle position, token ~93,500–110,500).
Note: Topic B runs turns 31–60. Facts are planted late in Topic B to maximize middle-context stress — turns 55–60 land squarely in the vulnerable zone of a ~200k context window.

| Fact | Value |
|------|-------|
| Artwork title | The Annunciation of Forlì |
| Artist | Melozzo da Forlì |
| Patron | Cardinal Giuliano della Rovere |
| Completion year | 1483 |
| Pigment technique | Lead white ground with ultramarine glaze |

**Topic C — Macroeconomics / Monetary Policy (turns 61–90)**
Rich topic development. No late-session survival probe specific to Topic C — it tests bleed detection only.

| Fact | Value |
|------|-------|
| Central bank target | Federal Reserve |
| Interest rate model | Taylor Rule |
| Inflation threshold discussed | 2.3% |
| Economist referenced | Dr. Priya Mehta |
| Policy instrument | Reverse repurchase agreements |

**Topic D — Marine Biology (turns 91–120)**
Planted in turns 100–110 (late position, token ~170,000–187,000).

| Fact | Value |
|------|-------|
| Species studied | Vampyroteuthis infernalis (vampire squid) |
| Depth range | 600–900 meters |
| Researcher | Dr. Kenji Watanabe |
| Bioluminescence organ | Photophores along mantle margin |
| Feeding behavior | Marine snow particle aggregation |

### Behavioral Rules
Established turns 1–2. Detectable by automatic rule detection mechanism.
- Rule 1: Numbered lists for technical specifications
- Rule 2: Risk classification at end of structural recommendations

Note: Rule 2 is domain-specific to Topic A. In Topics B, C, D the model will not receive prompts requiring structural recommendations, so rule compliance is naturally tested only in Topic A contexts. Late-turn probes explicitly ask the model to state both rules regardless of topic.

### Conversation Structure

| Turns | Phase | Content |
|-------|-------|---------|
| 1–2 | Rule establishment | Two formatting rules established explicitly |
| 3–5 | Early plant | All five Topic A facts planted |
| 6–30 | Topic A development | Bridge design, load analysis, material selection, risk assessment |
| 31–54 | Topic B early | Renaissance context, patronage systems, technique analysis |
| 55–60 | Middle plant | All five Topic B facts planted (middle-context stress position) |
| 61–90 | Topic C | Monetary policy, Taylor Rule, rate decisions, inflation models |
| 91–99 | Topic D early | Deep sea ecosystems, pressure adaptation |
| 100–110 | Late plant | All five Topic D facts planted |
| 111 | Topic D continued | One more substantive Topic D question |
| 112–120 | Rubric probes | All planted facts tested, topic bleed checked, rules verified |

### Response Length Targeting

Questions are designed to elicit responses of approximately 1,500 tokens. Techniques:
- "Walk me through step by step..."
- "Compare and contrast in detail..."
- "Analyze the following from first principles..."
- "What are all the factors that contribute to..."
- "Explain the full methodology for..."

At ~1,700 tokens per turn pair × 120 turns = ~204,000 tokens for Condition A at peak. This lands the middle-planted facts (turns 55–60) at token position ~93,500–102,000 in a ~204,000 token context — squarely in the lost-in-the-middle vulnerable zone.
...
```json
{
  ...
  "system_prompt": "You are a helpful technical and research assistant with deep knowledge across engineering, art history, economics, and biology. When listing technical specifications or multiple items, always format them as a numbered list. Always conclude structural or engineering recommendations with a risk classification in parentheses: (Risk: Low), (Risk: Medium), or (Risk: High). Apply these rules consistently throughout our entire conversation.",
  "turns": [
    {
      "turn": 1,
...
- [ ] `experiments/study_002/script.json` committed with exactly 120 turns
- [ ] `experiments/study_002/rubric_filled.md` committed
- [ ] Early facts planted in turns 3–5 (Topic A)
- [ ] Middle facts planted in turns 55–60 (Topic B, middle-context stress position)
- [ ] Late facts planted in turns 100–110 (Topic D)
- [ ] Behavioral rules established in turns 1–2, detectable by keyword
- [ ] Rubric probe turns at 112–120
- [ ] Three named researchers — one per domain — present for disambiguation
...
| ID | Description |
|----|-------------|
| S2-T-057 | Create `experiments/study_002/rubric_filled.md` |
| S2-T-058 | Create `experiments/study_002/script.json` with all 120 turns |
| S2-T-059 | Verify all rubric questions map to specific script turns |
| S2-T-060 | Verify all three planted positions contain minimum five verifiable facts |
| S2-T-061 | Verify rule-setting turns 1–2 contain detectable rule language |
| S2-T-062 | Commit and push S2_007 |

---

## Notes

**The middle plant is the study's centerpiece.** Facts planted in turns 55–60 land at approximately token position 93,500–102,000 in a ~204,000 token full-context window. This is squarely in the lost-in-the-middle zone. If Condition A fails to recall these facts and Condition C succeeds, the study has demonstrated the architecture's primary value proposition.

**Three named researchers enable clean disambiguation.** Dr. Anara Bekova (engineering), Dr. Priya Mehta (economics), Dr. Kenji Watanabe (marine biology) — each in a distinct domain, each with a distinctive name. Q10 tests whether the model can correctly attribute each researcher to their domain after 120 turns of conversation across all four topics.

**The script is verbose by design.** Questions are crafted to elicit 1,500-token responses. This is not padding — it is the mechanism for reaching ~200k tokens in Condition A. The depth of questions is also scientifically valid: rich technical exchanges are precisely the use case where long-context memory matters most.